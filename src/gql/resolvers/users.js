...
export default {
	Query: {
		listAllUsers: async (parent, args, context) => {
			...
			const user = await context.di.model.Users.find().populate({
				path: 'userRole',
				select: 'name _id',
			});
			return user;
		...
		listAllAdminUsers: async (parent, args, context) => {
			...
			const user = await context.di.model.Users.aggregate([
				{
					$lookup: {
		...
		getUser: async (parent, args, context) => {
			context.di.authValidation.ensureThatUserIsLogged(context);
			const user = await context.di.model.Users.findById(context.user._id)
				.populate({
					path: 'userRole',
					select: 'name _id',
					populate: {
	...
	Mutation: {
		...
		updateUser: async (parent, args, context) => {
			...
			const isExist = await context.di.model.Users.findById(_id).lean();
